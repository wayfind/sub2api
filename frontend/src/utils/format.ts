...
//
// ===========================================================================
// 系统单位不变量执行点（Single Source of Truth）
// ===========================================================================
// 本文件的 USD_TO_U / uToUsdRound / usdToURound / formatUsdFromU 是**唯一**
// 允许做 U ↔ USD 单位转换的地方。
//
// 任何在其他文件里出现的 inline `× 70` / `÷ 70` / `* USD_TO_U` 都应该被替换
// 为调用这里的 helper。这是 L1 记忆 billing_architecture.md 里"全 U 无例外"
// 原则的代码层面执行点。
//
// 如果你需要新的转换函数，加在这里。不要在调用方写 inline 转换——这是历史上
// 多次 70 倍计费偏差事故的根因（见 L1 project_biases.md "USD/U 转换事故"）。
// ===========================================================================
 ...
 *
 * 已知 UX 问题：精度按值大小自动切换 (>=1 用 2 位, >=0.01 用 4 位, <0.01 用 6 位)。
 * 在 QuotaBadge 等同时显示 used/limit 的场景下，两个相邻的金额可能精度不一致
 * （如 "$0.5000 / $1142.86"），看着对不齐。同样，输入框中用户输入 "$0.05" 会被
 * 回显为 "$0.0500"——四位 vs 用户输入的两位，让人疑惑。
 *
 * 未来的改进方向：把精度策略作为参数传入，让调用方按上下文决定（统计区/输入区/
 * 列表区）。
 * 触发时机：下次有用户反馈"输入 $0.05 显示成 $0.0500 很怪"或者 QuotaBadge 的
 * 对齐被设计师挑刺时，再做这个抽离。在那之前现有行为是 acceptable 的——金额值
 * 的差异比格式不齐更影响信任。
 */
